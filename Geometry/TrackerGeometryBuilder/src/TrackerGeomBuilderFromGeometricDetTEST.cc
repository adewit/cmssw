#include "Geometry/TrackerGeometryBuilder/interface/TrackerGeomBuilderFromGeometricDetTEST.h"
...
TrackerGeometry* TrackerGeomBuilderFromGeometricDetTEST::build(const GeometricDet* gd,
                                                           const PGeometricDetExtra* gdex,
                                                           const PTrackerParameters& ptp,
                                                           const TrackerTopology* tTopo) {
  if (ptp.vpars.size() != 6) {
    throw cms::Exception("TrackerGeomBuilderFromGeometricDetTEST")
  ...
  for (unsigned int i = 0; i < 6; ++i) {
    if (gdsubdetmap[i] == GeometricDet::PixelBarrel)
      buildPixel(
          dets[i], gdex,tracker, GeomDetEnumerators::SubDetector::PixelBarrel, false, BIG_PIX_PER_ROC_X, BIG_PIX_PER_ROC_Y);
    if (gdsubdetmap[i] == GeometricDet::PixelPhase1Barrel)
      buildPixel(dets[i], gdex,tracker, GeomDetEnumerators::SubDetector::P1PXB, false, BIG_PIX_PER_ROC_X, BIG_PIX_PER_ROC_Y);
    // Phase2 case
    if (gdsubdetmap[i] == GeometricDet::PixelPhase2Barrel)
      buildPixel(dets[i], gdex,tracker, GeomDetEnumerators::SubDetector::P2PXB, true, BIG_PIX_PER_ROC_X, BIG_PIX_PER_ROC_Y);
    //
    if (gdsubdetmap[i] == GeometricDet::PixelEndCap)
      buildPixel(
          dets[i], gdex,tracker, GeomDetEnumerators::SubDetector::PixelEndcap, false, BIG_PIX_PER_ROC_X, BIG_PIX_PER_ROC_Y);
    if (gdsubdetmap[i] == GeometricDet::PixelPhase1EndCap)
      buildPixel(
          dets[i],gdex, tracker, GeomDetEnumerators::SubDetector::P1PXEC, false, BIG_PIX_PER_ROC_X, BIG_PIX_PER_ROC_Y);
    if (gdsubdetmap[i] == GeometricDet::PixelPhase2EndCap)
      buildPixel(dets[i], gdex, tracker, GeomDetEnumerators::SubDetector::P2PXEC, true, BIG_PIX_PER_ROC_X, BIG_PIX_PER_ROC_Y);
    if (gdsubdetmap[i] == GeometricDet::OTPhase2Barrel)
      buildPixel(dets[i], gdex,tracker, GeomDetEnumerators::SubDetector::P2OTB, true, BIG_PIX_PER_ROC_X, BIG_PIX_PER_ROC_Y);
    if (gdsubdetmap[i] == GeometricDet::OTPhase2EndCap)
      buildPixel(dets[i], gdex,tracker, GeomDetEnumerators::SubDetector::P2OTEC, true, BIG_PIX_PER_ROC_X, BIG_PIX_PER_ROC_Y);
  }
  //now building Strips
  for (unsigned int i = 0; i < 6; ++i) {
...
void TrackerGeomBuilderFromGeometricDetTEST::buildPixel(
    std::vector<const GeometricDet*> const& gdv,
    const PGeometricDetExtra* const& gdve,
    ...
    int BIG_PIX_PER_ROC_Y)  // in y direction, cols. BIG_PIX_PER_ROC_Y = 0 for SLHC
{
  ...
  for (auto const& i : gdv) {
    ...
    std::cout<<"Size of gdve pgdes "<<gdve->pgdes_.size()<<std::endl;
    for (auto const& j :gdve->pgdes_){ 
        std::cout<<"we have "<<j._isBricked<<std::endl;
    }
    bool isBricked = true;
...
void TrackerGeomBuilderFromGeometricDetTEST::buildSilicon(std::vector<const GeometricDet*> const& gdv,
                                                      TrackerGeometry* tracker,
                                                      GeomDetType::SubDetector det,
...
void TrackerGeomBuilderFromGeometricDetTEST::buildGeomDet(TrackerGeometry* tracker) {
...
PlaneBuilderFromGeometricDet::ResultType TrackerGeomBuilderFromGeometricDetTEST::buildPlaneWithMaterial(
